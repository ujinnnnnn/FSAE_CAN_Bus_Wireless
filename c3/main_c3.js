...
function graphNewDataC3(this_dataset,dataAsString,this_chart){
  var x =  (((new Date()).getTime() - epochTime)/1000).toFixed(2),y = parseFloat(dataAsString)
  if (this_dataset[0].length > 100) {
      this_dataset[0].pop();
      this_dataset[0].append(x);
      this_dataset[1].pop();
      this_dataset[1].append(y);
  } else {
      this_dataset[0].append(x);
      this_dataset[1].append(y);
  }
  this_chart.load(this_dataset);
}
...
var hv_voltage_chart = c3.generate({
  bindto:  '#hv_voltage_chart',
  data : {
    x: 'x',
    columns : hv_voltage_data
  },
});
  ...
  for (const key in data){
    switch (key){
      ...
      case "inv_voltage":
        ...
        var x =  Math.round(((new Date()).getTime() - epochTime)/100),y = parseFloat(data[key])
        if (hv_voltage_data[0].length > 100) {
            hv_voltage_data[0].pop();
            hv_voltage_data[1].pop();
            hv_voltage_data[2].pop();
            hv_voltage_data[0].push(x);
            hv_voltage_data[1].push(y);
            hv_voltage_data[2].push(hv_voltage_data[2].slice(-1));
        } else {
            hv_voltage_data[0].push(x);
            hv_voltage_data[1].push(y);
            hv_voltage_data[2].push(hv_voltage_data[2][hv_voltage_data[2].length - 1]);
        }
        hv_voltage_chart.load(hv_voltage_data);
        console.log(hv_voltage_data);
        break;
